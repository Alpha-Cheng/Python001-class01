...
3. 数据清洗后，再次存储的数据不应出现缺失值。    v                                 # 第二遍处理评论后第二此存储
4. Django 能够正确运行，并展示采集到的数据，数据不应该有乱码、缺失等问题。  v        # 第二遍分析和展示评论
5. 在 Django 上采用图表方式展示数据分类情况。    v                                # 第二遍展示评论分析结论数据
6. 舆情分析的结果存入到 MySQL 数据库中。     v                                    # 第二遍展示评论分析结论数据
7. 在 Django 上采用图表方式展示舆情分析的结果。    v                              # 第二遍展示评论分析结论数据
